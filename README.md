...
./faceid.py run --show
```
Note, that displaying videos streams and recognized faces on them (the `--show` option) dramatically slows down the whole system, so you should use it for debug purposes only. In order to initialize video devices using configuration files `cfg/video/*.json`, run the following command:
```bash
